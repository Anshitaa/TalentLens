-- Runs automatically inside Docker on first start.
-- POSTGRES_USER=talentlens already exists (created by Docker env vars).
-- This file only creates schemas and tables.
...
CREATE INDEX IF NOT EXISTS idx_raw_emp_dept    ON raw.hris_employee_snapshot(department);
CREATE INDEX IF NOT EXISTS idx_raw_emp_active  ON raw.hris_employee_snapshot(is_active);
CREATE INDEX IF NOT EXISTS idx_raw_emp_mgr     ON raw.hris_employee_snapshot(manager_id);
...
CREATE INDEX IF NOT EXISTS idx_raw_evt_employee ON raw.employee_events(employee_id);
CREATE INDEX IF NOT EXISTS idx_raw_evt_type     ON raw.employee_events(event_type);
CREATE INDEX IF NOT EXISTS idx_raw_evt_date     ON raw.employee_events(event_date);
...
CREATE INDEX IF NOT EXISTS idx_raw_funnel_cand  ON raw.hiring_funnel_events(candidate_id);
...
CREATE TABLE IF NOT EXISTS mart.feature_store (
    ...
    monthly_income_vs_peer_median   NUMERIC(5,4),
    income_peer_percentile          SMALLINT,
    performance_rating_delta        NUMERIC(5,2),
...
CREATE INDEX IF NOT EXISTS idx_fact_scores_emp    ON mart.fact_risk_scores(employee_id);
CREATE INDEX IF NOT EXISTS idx_fact_scores_at     ON mart.fact_risk_scores(scored_at);
...
CREATE INDEX IF NOT EXISTS idx_audit_dec_emp ON audit.model_decisions(employee_id);
CREATE INDEX IF NOT EXISTS idx_audit_dec_at  ON audit.model_decisions(decision_at);
...
CREATE TABLE IF NOT EXISTS audit.active_learning_labels (
    ...
    corrected_label     SMALLINT    NOT NULL,
